...
class MobaMO(pufferlib.PufferEnv):
    def __init__(self, num_envs=4, vision_range=5, agent_speed=1.0,
            ...
            freeze_on_done=False, max_ticks=0,
            script_opponents=True, render_mode='human', buf=None, seed=0, gamma=0.0):
        ...
        if self.freeze_on_done:
            ...
            self.done_envs = [False for _ in range(self.num_envs)]
    ...
    def step(self, actions):
        ...
        if self.freeze_on_done:
            ...
            for i in range(self.num_envs):
                if self.done_envs[i]:
                    continue
                done = self.terminals[i*p:(i+1)*p].all() or self.truncations[i*p:(i+1)*p].all()
                if done:
                    self.done_envs[i] = True
                    log = binding.vec_log_single(self.c_envs, i)
                    infos[i] = log or {}
        elif self.tick % self.report_interval == 0:
