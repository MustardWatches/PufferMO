...
typedef struct MOBAMONet MOBAMONet;
struct MOBAMONet {
    int num_agents;
    float* obs_2d;
    float* obs_1d;
...
MOBAMONet* init_mobamonet(Weights* weights, int num_agents) {
    MOBAMONet* net = calloc(1, sizeof(MOBAMONet));
    int hidden = 128;
...
void free_mobamonet(MOBAMONet* net) {
    free(net->obs_2d);
    free(net->obs_1d);
    free(net->conv1);
    free(net->relu1);
...
void forward(MOBAMONet* net, unsigned char* observations, int* actions) {
...
void demo() {
    ...
    MOBAMONet* net = init_mobamonet(weights, num_agents);
...
void test_bugs(float test_time) {
    Weights* weights = load_weights("resources/moba/moba_weights.bin", 380056);
    MOBAMONet* net = init_mobamonet(weights, 10);
