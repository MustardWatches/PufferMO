...
static PyObject* vec_log_single(PyObject* self, PyObject* args) {
    VecEnv* vec = unpack_vecenv(args);
    if (!vec) {
        return NULL;
    }

    PyObject* i_arg = PyTuple_GetItem(args, 1);
    if (!PyObject_TypeCheck(i_arg, &PyLong_Type)) {
        PyErr_SetString(PyExc_TypeError, "i must be an integer");
        return NULL;
    }
    int i = PyLong_AsLong(i_arg);
    if (i < 0 || i >= vec->num_envs) {
